      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid md:grid-cols-3 gap-12">
          {skillCategories.map(({ title, skills }, categoryIndex) => (
            ...
            >
              <h3 className="text-2xl font-semibold mb-8 text-accent">{title}</h3>
              <div className="flex flex-wrap justify-center gap-3">
                {skills.map((skill, skillIndex) => (
                  <Badge
                    key={skill}
                    variant="secondary"
                    className="px-4 py-2 text-sm hover:scale-105 transition-all duration-300 hover:glow-accent cursor-default"
